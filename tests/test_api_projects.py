...
class TestCreateProject:
    ...
    async def test_create_project_full(self, client: AsyncClient):
        payload = {"name": "Full Project", "description": "Detailed description", "color": "#123456"}
    ...
    async def test_create_project_invalid_color(self, client: AsyncClient):
        resp = await client.post("/api/v1/projects", json={"name": "Bad Color", "color": "red"})
        assert resp.status_code == 422

    async def test_create_project_valid_color_formats(self, client: AsyncClient):
        resp = await client.post("/api/v1/projects", json={"name": "Color Test", "color": "#AABBCC"})
        assert resp.status_code == 201
        resp2 = await client.post("/api/v1/projects", json={"name": "Color Test 2", "color": "#aabbcc"})
...
class TestGetProject:
    ...
    async def test_get_project_includes_tasks(self, client: AsyncClient, sample_project, sample_task):
        resp = await client.get(f"/api/v1/projects/{sample_project.id}")
        assert resp.status_code == 200
        data = resp.json()
    ...
    async def test_get_project_empty_tasks(self, client: AsyncClient, sample_project):
        resp = await client.get(f"/api/v1/projects/{sample_project.id}")
        data = resp.json()
        assert isinstance(data["tasks"], list)

    async def test_get_project_tasks_contain_task_fields(self, client: AsyncClient, sample_project, sample_task):
        resp = await client.get(f"/api/v1/projects/{sample_project.id}")
...
class TestUpdateProject:
    async def test_update_project_name(self, client: AsyncClient, sample_project):
        resp = await client.patch(f"/api/v1/projects/{sample_project.id}", json={"name": "Renamed Project"})
        assert resp.status_code == 200
        assert resp.json()["name"] == "Renamed Project"

    async def test_update_project_description(self, client: AsyncClient, sample_project):
        resp = await client.patch(f"/api/v1/projects/{sample_project.id}", json={"description": "New description"})
        assert resp.status_code == 200
        assert resp.json()["description"] == "New description"

    async def test_update_project_color(self, client: AsyncClient, sample_project):
        resp = await client.patch(f"/api/v1/projects/{sample_project.id}", json={"color": "#000000"})
        assert resp.status_code == 200
        assert resp.json()["color"] == "#000000"

    async def test_update_project_not_found(self, client: AsyncClient):
        resp = await client.patch("/api/v1/projects/99999", json={"name": "Ghost"})
        assert resp.status_code == 404

    async def test_update_project_duplicate_name(self, client: AsyncClient, sample_project):
        other = await client.post("/api/v1/projects", json={"name": "Other Project"})
        assert other.status_code == 201
        resp = await client.patch(f"/api/v1/projects/{sample_project.id}", json={"name": "Other Project"})
        assert resp.status_code == 409

    async def test_update_project_same_name_ok(self, client: AsyncClient, sample_project):
        resp = await client.patch(f"/api/v1/projects/{sample_project.id}", json={"name": sample_project.name})
        assert resp.status_code == 200

    async def test_update_project_partial(self, client: AsyncClient, sample_project):
        original_color = sample_project.color
        resp = await client.patch(f"/api/v1/projects/{sample_project.id}", json={"description": "Updated"})
        assert resp.status_code == 200
        data = resp.json()
        assert data["description"] == "Updated"
        assert data["color"] == original_color

    async def test_update_project_invalid_color(self, client: AsyncClient, sample_project):
        resp = await client.patch(f"/api/v1/projects/{sample_project.id}", json={"color": "not-a-color"})
        assert resp.status_code == 422
...
class TestDeleteProject:
    ...
    async def test_delete_project_removes_from_list(self, client: AsyncClient, sample_project):
        await client.delete(f"/api/v1/projects/{sample_project.id}")
        resp = await client.get("/api/v1/projects")
        assert resp.json() == []

    async def test_delete_project_unlinks_tasks(self, client: AsyncClient, sample_project, sample_task):
        await client.delete(f"/api/v1/projects/{sample_project.id}")
...
class TestTagEndpoints:
    ...
    async def test_create_tag(self, client: AsyncClient):
        resp = await client.post("/api/v1/tags", json={"name": "backend", "color": "#0000FF"})
        assert resp.status_code == 201
        data = resp.json()
    ...
    async def test_update_tag(self, client: AsyncClient, sample_tag):
        resp = await client.patch(f"/api/v1/tags/{sample_tag.id}", json={"name": "critical"})
        assert resp.status_code == 200
