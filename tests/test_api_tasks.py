...
from app.models import Task, Project, Tag, TaskStatus, Priority
...
class TestListTasks:
    ...
    async def test_list_tasks_filter_by_priority(self, client: AsyncClient, multiple_tasks):
    ...
    async def test_list_tasks_filter_by_project(self, client: AsyncClient, multiple_tasks, sample_project):
    ...
    async def test_list_tasks_search_case_insensitive(self, client: AsyncClient, multiple_tasks):
        resp = await client.get("/api/v1/tasks?search=search")
        assert resp.status_code == 200
        data = resp.json()
        assert data["total"] == 1

    async def test_list_tasks_filter_by_tag(self, client: AsyncClient, sample_task, sample_tag):
        resp_tag = await client.post("/api/v1/tasks", json={
            "title": "Tagged Task",
    ...
    async def test_list_tasks_combined_filters(self, client: AsyncClient, multiple_tasks, sample_project):
        resp = await client.get(f"/api/v1/tasks?status=todo&project_id={sample_project.id}")
        assert resp.status_code == 200
        data = resp.json()
        assert data["total"] == 1
...
class TestCreateTask:
    ...
    async def test_create_task_full(self, client: AsyncClient, sample_project, sample_tag):
        payload = {
            "title": "Full Task",
    ...
    async def test_create_task_invalid_priority(self, client: AsyncClient):
        resp = await client.post("/api/v1/tasks", json={"title": "Task", "priority": "extreme"})
        assert resp.status_code == 422

    async def test_create_task_invalid_status(self, client: AsyncClient):
        resp = await client.post("/api/v1/tasks", json={"title": "Task", "status": "unknown"})
        assert resp.status_code == 422

    async def test_create_task_nonexistent_tag(self, client: AsyncClient):
        resp = await client.post("/api/v1/tasks", json={"title": "Task", "tag_ids": [9999]})
        assert resp.status_code == 404

    async def test_create_task_with_multiple_tags(self, client: AsyncClient, sample_tag, sample_tag2):
        resp = await client.post("/api/v1/tasks", json={
            "title": "Multi-tag task",
            "tag_ids": [sample_tag.id, sample_tag2.id],
        })
        assert resp.status_code == 201
        data = resp.json()
    ...
    async def test_create_task_all_priorities(self, client: AsyncClient):
        for priority in ["low", "medium", "high", "urgent"]:
            resp = await client.post("/api/v1/tasks", json={"title": f"Task {priority}", "priority": priority})
            assert resp.status_code == 201
    ...
    async def test_create_task_all_statuses(self, client: AsyncClient):
        for s in ["todo", "in_progress", "done", "cancelled"]:
            resp = await client.post("/api/v1/tasks", json={"title": f"Task {s}", "status": s})
            assert resp.status_code == 201
...
class TestGetTask:
    ...
    async def test_get_task_includes_project(self, client: AsyncClient, sample_task, sample_project):
        resp = await client.get(f"/api/v1/tasks/{sample_task.id}")
        assert resp.status_code == 200
        data = resp.json()
    ...
    async def test_get_task_includes_tags(self, client: AsyncClient, sample_task, sample_tag):
        await client.patch(f"/api/v1/tasks/{sample_task.id}", json={"tag_ids": [sample_tag.id]})
        resp = await client.get(f"/api/v1/tasks/{sample_task.id}")
        assert resp.status_code == 200
        data = resp.json()
        assert len(data["tags"]) == 1
...
class TestUpdateTask:
    async def test_update_task_title(self, client: AsyncClient, sample_task):
        resp = await client.patch(f"/api/v1/tasks/{sample_task.id}", json={"title": "Updated Title"})
        assert resp.status_code == 200
        assert resp.json()["title"] == "Updated Title"

    async def test_update_task_status(self, client: AsyncClient, sample_task):
        resp = await client.patch(f"/api/v1/tasks/{sample_task.id}", json={"status": "in_progress"})
        assert resp.status_code == 200
        assert resp.json()["status"] == "in_progress"

    async def test_update_task_priority(self, client: AsyncClient, sample_task):
        resp = await client.patch(f"/api/v1/tasks/{sample_task.id}", json={"priority": "urgent"})
        assert resp.status_code == 200
        assert resp.json()["priority"] == "urgent"

    async def test_update_task_to_done_sets_completed_at(self, client: AsyncClient, sample_task):
        resp = await client.patch(f"/api/v1/tasks/{sample_task.id}", json={"status": "done"})
        assert resp.status_code == 200
        data = resp.json()
        assert data["status"] == "done"
        assert data["completed_at"] is not None

    async def test_update_task_from_done_clears_completed_at(self, client: AsyncClient, sample_task):
        await client.patch(f"/api/v1/tasks/{sample_task.id}", json={"status": "done"})
        resp = await client.patch(f"/api/v1/tasks/{sample_task.id}", json={"status": "todo"})
        assert resp.status_code == 200
        data = resp.json()
        assert data["status"] == "todo"
        assert data["completed_at"] is None

    async def test_update_task_tags(self, client: AsyncClient, sample_task, sample_tag, sample_tag2):
        resp = await client.patch(f"/api/v1/tasks/{sample_task.id}", json={"tag_ids": [sample_tag.id, sample_tag2.id]})
        assert resp.status_code == 200
        data = resp.json()
        assert len(data["tags"]) == 2

    async def test_update_task_clear_tags(self, client: AsyncClient, sample_task, sample_tag):
        await client.patch(f"/api/v1/tasks/{sample_task.id}", json={"tag_ids": [sample_tag.id]})
        resp = await client.patch(f"/api/v1/tasks/{sample_task.id}", json={"tag_ids": []})
        assert resp.status_code == 200
        assert resp.json()["tags"] == []

    async def test_update_task_nonexistent_tag(self, client: AsyncClient, sample_task):
        resp = await client.patch(f"/api/v1/tasks/{sample_task.id}", json={"tag_ids": [9999]})
        assert resp.status_code == 404
    ...
    async def test_update_task_partial(self, client: AsyncClient, sample_task):
        ...
        resp = await client.patch(f"/api/v1/tasks/{sample_task.id}", json={"title": "Only Title Changed"})
        assert resp.status_code == 200
        data = resp.json()
    ...
    async def test_update_task_project(self, client: AsyncClient, sample_task):
        new_proj_resp = await client.post("/api/v1/projects", json={"name": "New Project"})
        assert new_proj_resp.status_code == 201
        new_proj_id = new_proj_resp.json()["id"]
        resp = await client.patch(f"/api/v1/tasks/{sample_task.id}", json={"project_id": new_proj_id})
        assert resp.status_code == 200
        assert resp.json()["project"]["id"] == new_proj_id

    async def test_update_task_unset_project(self, client: AsyncClient, sample_task):
        resp = await client.patch(f"/api/v1/tasks/{sample_task.id}", json={"project_id": None})
        assert resp.status_code == 200
        assert resp.json()["project"] is None

    async def test_update_task_invalid_status(self, client: AsyncClient, sample_task):
        resp = await client.patch(f"/api/v1/tasks/{sample_task.id}", json={"status": "invalid"})
        assert resp.status_code == 422
...
class TestDeleteTask:
    ...
    async def test_delete_task_removes_from_list(self, client: AsyncClient, multiple_tasks):
        task_id = multiple_tasks[0].id
        await client.delete(f"/api/v1/tasks/{task_id}")
        resp = await client.get("/api/v1/tasks")
        assert resp.json()["total"] == 4

    async def test_delete_task_with_tags_preserves_tags(self, client: AsyncClient, sample_task, sample_tag):
        await client.patch(f"/api/v1/tasks/{sample_task.id}", json={"tag_ids": [sample_tag.id]})
