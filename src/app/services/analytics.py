...
- overdue_count: tasks with a due_date in the past that are not done
- avg_completion_time_hours: average hours between created_at and completed_at
  for tasks with status == done and a recorded completed_at

Productivity stats
------------------
- completed_per_day  : {date_str -> count} for completed tasks grouped by
                        completed_at date (UTC), last 30 days
- completed_per_week : {iso_week_str -> count} for completed tasks grouped by
